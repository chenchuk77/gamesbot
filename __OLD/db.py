...
class Club(Model):
    ...
    total_profit = DecimalField(decimal_places=2)
...
class Game(Model):
    id = AutoField()
    buyin = CharField() ### (70+70+12) or (100+15)
    total_price = DecimalField(decimal_places=2) ### int sum of the above
    bounty = BooleanField(default=False)
    win = DecimalField(decimal_places=2)
    club = ForeignKeyField(Club, backref='games')
    # type = CharField(choices=['NLH','PLO', 'PLO5', ''])
