...
# In [48]: timedelta(days = 25)
# Out[48]: datetime.timedelta(days=25)
#
# In [49]: datetime.datetime.now() > g.started
# Out[49]: True
#
# In [50]: datetime.datetime.now() > g.started + timedelta(days = 1)
# Out[50]: False
#
# In [51]:
...
def get_games_last_n_days(n):
    if n < 1:
        return []
    # n=1 means today results   (games from today 08:00 AM)
    # n=2 means today+testerday (games from yesterday 08:00 AM)
    now = datetime.datetime.utcnow().date()
    last_0800_am = datetime.datetime.fromisoformat(now.isoformat() + ' 08:00')

    start_point = last_0800_am - timedelta(days=n-1)
    session = Session()
...
def get_today_buyin():
    today_games=get_games_last_n_days(1)

#
# def get_today_buyins():
#     session = Session()
#     db_games = session.query(Game)
