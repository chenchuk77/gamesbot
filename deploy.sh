...
ssh -i /opt/lms/keys/dev_access_key.pem ubuntu@freedomgpt "cd gamesbot && git pull && ./build-local-image.sh --q && git log --oneline | head -n 10 > ./git.log && ./stop-gamesbot.sh && ./start-gamesbot.sh"
